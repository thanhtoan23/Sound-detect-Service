...
\subsubsection{Mô tả chức năng}
\texttt{AudioClassifier} thực hiện phân loại âm thanh môi trường sử dụng các kỹ thuật học máy. Về mặt luồng xử lý, module nhận \textit{clean audio} từ DSP và tạo ra kết quả dự đoán dạng \textit{label} và \textit{confidence}. Để mô hình hoạt động ổn định trong thời gian thực, hệ thống sử dụng các cơ chế theo thời gian (temporal logic) thay vì suy luận độc lập trên từng chunk nhỏ.
...
\begin{itemize}
    ...
    \item \textbf{Buffering/Windowing}: tích lũy dữ liệu theo cửa sổ ngắn (vài giây) để tạo input đủ ``già'' cho mô hình.
