...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \end{itemize}
        \item \textbf{Output:} Tạo thư mục dataset con (ví dụ \textit{ESC50\_subset\_with\_unknown/}) và các file CSV split:
    ...
    \begin{itemize}
        ...
        \end{itemize}
        \item \textbf{Các phép biến đổi (ví dụ cấu hình):}
        \begin{itemize}
        ...
        \begin{itemize}
            \item \textit{TRAIN\_NOISE\_PROB}: xác suất trộn noise (ví dụ $0.7$).
            \item \textit{TRAIN\_NOISE\_SNR\_RANGE\_DB}: khoảng SNR (ví dụ $(10, 30)$ dB).
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Frequency masking: tối đa \textit{freq\_mask\_max = 14} bins.
    ...
    \begin{itemize}
        ...
        \item Sau cùng: \textit{Conv2D(256)} $\rightarrow$ \textit{BatchNorm} $\rightarrow$ \textit{ReLU} $\rightarrow$ \textit{GlobalAveragePooling} $\rightarrow$ \textit{Dropout(0.5)} $\rightarrow$ \textit{Dense(num\_classes, softmax)}.
    \end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \]
        \item Nếu \textit{rms < ENV\_MIN\_RMS} (ví dụ 0.005), hệ thống tránh đưa ra dự đoán mới để giảm false trigger trong vùng im lặng/nhiễu nhỏ.
    \end{itemize}
    ...
    \begin{itemize}
        \item Sau khi có nhãn dự đoán và xác suất tương ứng, nếu điểm dự đoán \textbf{bé hơn} \textit{ENV\_CONF\_THRESHOLD} thì output bị ép thành \textit{unknown}.
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Tham số: \textit{ENV\_PRED\_HOP\_SEC} (ví dụ 0.5s).
    ...
    \begin{itemize}
        \item Tham số: \textit{ENV\_SMOOTH\_K} (ví dụ $K=3$, thường 3--7 là hợp lý).
    ...
    \begin{itemize}
        \item Tham số: \textit{ENV\_RESET\_ON\_SILENCE\_SEC} (ví dụ 0.8s).
